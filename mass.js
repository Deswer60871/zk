...
var scriptData = {
	...
	helpLink: 'https://forum.tribalwars.net/index.php?threads/mass-command-timer.286017/',
};
...
var translations = {
	...
	sk_SK: {
		'Mass Command Timer': 'HromadnĂ˝ plĂˇnovaÄŤ',
		Help: 'Pomoc',
		'Select Unit': 'VybraĹĄ jednotku',
		'Enter Coordinates': 'ZadaĹĄ koordinĂˇty',
		'Calculate Times': 'VypoÄŤĂ­taĹĄ ÄŤasy',
		'Select Landing Time': 'ZadaĹĄ ÄŤas prĂ­chodu',
		'Coordinates field is empty!': 'KoordinĂˇty nie sĂş vyplnenĂ©!',
		'Target coordinates are invalid!': 'Target coordinates are invalid!',
		From: 'Z',
		To: 'Do',
		Distance: 'VzdialenosĹĄ',
		'Travel Time': 'Trvanie',
		'Launch Time': 'ÄŚas odoslania',
		Send: 'OdoslaĹĄ',
		'No village can reach in time!': 'Jednotky zo Ĺľiadnej dediny nestihnĂş doraziĹĄ vÄŤas!',
		'This script requires Premium Account to be active!': 'Tento skript vyĹľaduje aktĂ­vny prĂ©miovĂ˝ ĂşÄŤet!',
		'possible combinations found': 'moĹľnĂ˝ch kombinĂˇciĂ­ nĂˇjdenĂ˝ch',
		Unit: 'Jednotka',
		Status: 'Stav',
		'Commands List Export': 'ExportovaĹĄ zoznam prĂ­kazov',
		'Landing Time': 'ÄŚas prĂ­chodu',
		Command: 'PrĂ­kaz',
		'Send In': 'Send In',
		'Export Plan': 'Export Plan',
		'Reset Config': 'Reset Config',
	...
	pt_PT: {
		...
		'Coordinates field is empty!': 'O espaĂ§o das coordenadas estĂˇ vazio!',
		From: 'De',
		To: 'Para',
		Distance: 'DistĂ˘ncia',
		...
		Send: 'Enviar',
		'No village can reach in time!': 'Nenhuma aldeia consegue chegar a tempo',
		'This script requires Premium Account to be active!':
			'Este script precisa de conta premium ativa para funcionar!',
		'possible combinations found': 'combinaĂ§Ăµes possĂ­veis encontradas',
		Unit: 'Unidade',
		Status: 'Estado',
	...
	tr_TR: {
		'Mass Command Timer': 'Toplu Komut ZamanlayÄ±cÄ±',
		Help: 'YardÄ±m',
		'Select Unit': 'Birim SeĂ§',
		'Enter Coordinates': 'KoodinatlarÄ± Girin',
		'Calculate Times': 'ZamanlarÄ± Hesapla',
		'Select Landing Time': 'VarÄ±Ĺź ZamanÄ±nÄ± SeĂ§in',
		'Coordinates field is empty!': 'Koordinatlar alanÄ± boĹź!',
		From: 'dan',
		To: 'a',
		Distance: 'Mesafe',
		'Travel Time': 'Seyehat SĂĽresi',
		'Launch Time': 'BaĹźlatma ZamanÄ±',
		Send: 'GĂ¶nder',
		'No village can reach in time!': 'HiĂ§bir kĂ¶y zamanÄ±nda ulaĹźamaz!',
		'This script requires Premium Account to be active!':
			'Bu komut dosyasÄ±, Premium HesabÄ±n etkin olmasÄ±nÄ± gerektirir!',
		'possible combinations found': 'bulunan olasÄ± kombinasyonlar',
		Unit: 'Birim',
		Status: 'Durum',
		'Commands List Export': 'Komut Listesini DÄ±Ĺźa Aktar',
		'Landing Time': 'VarÄ±Ĺź ZamanÄ±',
		Command: 'Komut',
		'Send In': 'Send In',
		'Export Plan': 'Export Plan',
		'Reset Config': 'Reset Config',
...
function initMassCommandTimer() {
	...
	const content = `
        ...
        <div class="ra-mb15">
            ...
            <textarea class="ra-textarea" id="raCoordinates">${targets ?? ''}</textarea>
        </div>
	...
	setTimeout(function () {
		...
		const landingTimeIsNotEmpty = landingTime !== null && landingTime !== undefined;
...
function calculateTimes() {
	...
	const targetCoordinatesArray = jQuery('#raCoordinates').val().match(coord_regex);
	...
	if (DEBUG) {
		...
		console.debug(`${scriptInfo()} targetCoordinatesArray:`, targetCoordinatesArray);
	}
	...
	if (targetCoordinatesArray.length) {
		...
		targetCoordinatesArray.forEach((targetVillageCoord) => {
			ownVillagesCoords.forEach((ownVillageCoord) => {
				const distance = calculateDistance(targetVillageCoord, ownVillageCoord.coord);
				const launchTime = getLaunchTime(unit, landingTimeObject, distance);
				const formattedDistance = parseFloat(distance).toFixed(2);
				const unformattedTime = (landingTimeObject.getTime() - launchTime) / 1000;
				const formattedTime = secondsToHms(unformattedTime);
				const formattedLaunchTime = formatDateTime(launchTime);

				const chosenUnitTroopAmount = ownVillageCoord.troops[0][game_data.units.indexOf(unit)];
		...
		filteredTimes = filteredTimes.slice(0, 100000);
...
function exportCommands(commands, unit, landingTime) {
	...
	commands.forEach((command) => {
		const { startVillageId, start, destination, formattedLaunchTime } = command;
		...
		bbCodeTableRows += `\n[*]${formattedLaunchTime}[|]${tt('From')} ${start} ${tt('To')} ${destination} [url=${
			window.location.origin
		}${commandUrl}]${tt('Send')}[/url] [|]`;
	});

	const bbExport = `[size=12][b]${tt('Landing Time')}:[/b] ${landingTime} - [b]${tt(
		'Unit'
	)}:[/b] ${unit}[/size]\n\n[table][**]${tt('Launch Time')}[||]${tt('Command')}[||]${tt(
		'Status'
	)}[/**]${bbCodeTableRows}\n[/table]`;
...
function renderTable(commands) {
	...
	commands.forEach((command) => {
		...
		const timeTillLaunch = secondsToHms((unformattedLaunchTime - serverTime) / 1000);
...
function collectOwnVillages() {
	...
	combinedTable.each(function () {
		...
		const villageId = jQuery(this).find('.quickedit-content').parent().attr('data-id');
...
function preparePopupContent(popupBody, minWidth = '340px', maxWidth = '360px') {
	const popupHeader = `
		...
		<div class="ra-body">`;
	const popupFooter = `</div><small class="ra-footer"><strong>${tt(scriptData.name)} ${
		scriptData.version
	}</strong> - <a href="${scriptData.authorUrl}" target="_blank" rel="noreferrer noopener">${
		scriptData.author
	}</a> - <a href="${scriptData.helpLink}" target="_blank" rel="noreferrer noopener">${tt('Help')}</a></small>`;
	const popupStyle = `
		<style>
...
function writeStorage(data) {
	...
	localStorage.setItem(`${scriptData.prefix}_data`, JSON.stringify(dataToSave));
}
...
(function () {
	if (game_data.features.Premium.active) {
		...
		if (allowedScreens.includes(gameScreen) && allowedModes.includes(gameMode)) {
